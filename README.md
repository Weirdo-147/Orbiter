...
### Phase 0 — Foundation
- [x] Repository scaffolding and coding standards.
- [x] Local dev environment and test harness (initial skeleton).
- [x] Basic node identity + secure join tokens (design spec draft).
...
🚧 **Early project stage** — Phase 0 scaffolding is in place and core implementation is starting.

Immediate next steps:
1. Implement a minimal API server skeleton in `cmd/orbiter` and `internal/controlplane`.
2. Implement a worker heartbeat protocol in `cmd/orbiter-agent` and `internal/worker`.
3. Add first end-to-end deploy/health integration test.
4. Turn draft API contracts in `pkg/api` into versioned request/response models.

## Repository Layout

- `cmd/orbiter` — control-plane executable.
- `cmd/orbiter-agent` — worker-agent executable.
- `internal/controlplane` — scheduler/reconciler/control-plane internals.
- `internal/worker` — worker runtime internals.
- `pkg/api` — shared API contracts and types.
